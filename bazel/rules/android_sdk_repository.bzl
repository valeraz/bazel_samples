...
_SDK_TOOLS_URL = "https://dl.google.com/android/repository/commandlinetools-linux-{sdk_tools}.zip".format(sdk_tools = _ANDROID_SDK_TOOLS)
...
def _android_sdk_repository_impl(repo_ctx):
    ...
    repo_ctx.file(
        _INSTALL_TARGET_NAME + "/internal.bzl",
        content = """
def _install_sdk_impl(ctx):
    ...
    '''.format(sdkmanager = ctx.file._sdk_manager.path, env = "{java_home}"), is_executable = True)
...
install_sdk = rule(
    ...
    executable = True,
        ...
        """.format(
            ...
            path = _SDK_ROOT_PATH,
            java_home = repo_ctx.os.environ.get("JAVA_HOME"),
        ),
        executable = False,
    )
...
def android_sdk_repository(name = None, workspace_name = None, api_level = None, build_tools_version = None):
    ...
    native.android_sdk_repository(
        # androidsdk is a hardcoded external repo in Starlark rules_android
        name = "androidsdk",
